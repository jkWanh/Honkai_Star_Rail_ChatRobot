...
with driver.session() as session:
    for i, record in tqdm(enumerate(data), desc="Processing records"):
        session.run("""
            MERGE (lc:Light_cones {id: $id})
            SET lc.name = $name, lc.desc = $desc, lc.icon = $icon, lc.big_pic = $big_pic, lc.quality = $quality, lc.destiny = $destiny
        """, id=record['id'], name=record['name'], desc=record['desc'], icon=record['icon'], big_pic=record['big_pic'],
                    quality=record['quality'], destiny=record['destiny'])
        ...
        for p in record['promote']:
            ...
            session.run("""
                MATCH (lc:Light_cones {id: $id}), (p:Promote) WHERE id(p) = $p_id
                CREATE (lc)-[:PROMOTES]->(p)
